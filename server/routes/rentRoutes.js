...
router.put("/update/:id", async (req, res) => {
  ...
  try {
    const updatedRent = await Rent.findByIdAndUpdate(rentId, rentData, {
      new: true,
    }); // Update directly
    if (!updatedRent) {
    ...
    res
      .status(200)
      .json({ message: "Rent updated successfully", data: updatedRent });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error updating rent", error: error.message });
  }
});
...
router.put("/updateStatus/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error updating rent status", error: error.message });
  }
});
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deleting rent", error: error.message });
  }
});
